...
% 1. LOAD FEATURES
load Fea_resnet % 432x2048
load Fea_eff    % 432x1280

% Combine to 3328 Dimensions for Fatima Dataset
Train1_raw = double([featuresTrain_eff, featuresTrain_resnet]); 
Test1_raw = double([featuresTest_eff, featuresTest_resnet]); 

All_Features = [Train1_raw; Test1_raw];

num_classes = 6; 
cat_label = {'Natural','Anger','Fear','Joy','Sadness','Surprise'};
fea_length = size(All_Features, 2);
disp(['Feature length of Fatima network is: ' num2str(fea_length)])

% Create Labels
Train_tar = repmat(1:num_classes, [72, 1]); Train_tar = Train_tar(:);
Test_tar = repmat(1:num_classes, [18, 1]); Test_tar = Test_tar(:);
All_Labels = [Train_tar; Test_tar];
...
for N1=1:3 % Iteration loop
    ...
    %% NOVELTY VISUALIZATION: Interaction Maps (Children Dots Mapping)
    if N1 == 1 
        ...
        for img_count = 1:4
            ...
            title(['MSG-FS Graph: Fatima Dataset']);
            hold off;
        end
    end

    %% PHASE 4: 5-FOLD CROSS VALIDATION (Exactly 80% Train, 20% Test per fold)
    ...
    for k = 1:5
        ...
        % Classifiers
        m1 = fitcknn(X_tr, Y_tr, 'NumNeighbors', 5); [l1, s1] = predict(m1, X_te);
        m2 = TreeBagger(50, X_tr, Y_tr, 'Method', 'classification'); [l2_str, s2] = predict(m2, X_te); l2 = str2double(l2_str);
        m3 = fitctree(X_tr, Y_tr); [l3, s3] = predict(m3, X_te);
        m4 = fitcecoc(X_tr, Y_tr); [l4, s4] = predict(m4, X_te);
        m5 = fitcnb(X_tr, Y_tr, 'DistributionNames', 'kernel'); [l5, s5] = predict(m5, X_te);
    ...
    True_Cat = categorical(All_True_Labels, 1:num_classes, cat_label);
    Pred_Cat = categorical(All_Ensemble_Preds, 1:num_classes, cat_label);
    h = confusionchart(True_Cat, Pred_Cat); h.Title = ['Confusion Matrix: Fatima Dataset (Iter ', num2str(N1), ')'];
    ...
    title('ROC Curve: Fatima Dataset', 'FontWeight', 'bold'); legend('show', 'Location', 'Best'); grid on;
end
...
disp('---------------------------------------------------------')
disp('FINAL STATISTICAL RESULTS (MEAN ± SD) - Fatima Dataset')
disp('---------------------------------------------------------')
